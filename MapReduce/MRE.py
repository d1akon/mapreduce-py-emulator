...
class _TreeNode:
    ...
    def collect(this, dic, fsort):
        # Each node of this tree represents an intermediate key and its associated list of values
        nodos = [this]
        while len(nodos) > 0:
            _tree = nodos[0]
            del(nodos[0])
            ...
            #if tmpTree <> None:
            # TODO: Review, when in a combiner function a new key is written (not generated by the mappers)
            # tmpTree becomes None
...
class _Context:
    ...
    def write(this, k, v):
        # TODO: When writing a combiner, check that the intermediate key is not changed
        ...
        if (this.__stage == 0):
            ...
            if (type(k) == ValuesIterator) or (type(v) == ValuesIterator):
                raise MRE_Exception("Cannot write the list of values. Iterate over it with a for loop and write the elements separately; or use the next() method if you know that the list of values has only one element.")
...
class ValuesIterator:
        ...
        def __iter__(this):
            if (this.__firstTime):
                ...
                raise MRE_Exception("Cannot iterate over the list of values more than once. You must perform all operations within a single for loop.")
