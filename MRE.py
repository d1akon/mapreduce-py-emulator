...
class _TreeNode:
    """
    This class represents a node in a binary tree used to store intermediate keys and their associated values.
    Each node has a key and a list of values linked to that key. Nodes can also have left and right children,
    forming a binary tree structure.

    Attributes:
    -----------
    __key : any comparable data type
        The key of the current node.
    
    __values : list of tuples (key, value)
        A list of values associated with the node's key.
    
    __left : _TreeNode
        Left child node (left subtree).
    
    __right : _TreeNode
        Right child node (right subtree).

    Methods:
    --------
    __init__(self, key, value):
        Initializes the _TreeNode with the specified key and value and sets the left and right children to None.
    ...
    add(self, value):
        Adds a value to the list of values for this node's key.

    count(self):
        Returns the number of values associated with the node's key.

    getAndEmptyValues(self):
        Returns and clears the list of values for this node.

    getAllValues(self, values):
        Recursively collects all values from the node and its children.

    __addOrUpdate(self, aTree, key, value, cmp):
        Adds or updates a key-value pair in the tree using the specified comparison function.

    collect(self, dic, fsort):
        Traverses the tree and collects values, storing them in the provided dictionary.
    """
